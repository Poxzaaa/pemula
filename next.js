...
browser: ['Daff Ni Boss','Safari','1.0.0'],
auth: state,
version
})
...
console.log(color(figlet.textSync(`SC DAFF4YOU`, {
font: 'Standard',
...
diablo.sendContact = async (jid, contact, quoted, opts) => {
      let list = []
      contact.map(v => list.push({
         displayName: v.name,
         vcard: `BEGIN:VCARD\nVERSION:3.0\nFN:${v.name}\nORG:Daff Ganteng\nTEL;type=CELL;type=VOICE;waid=${v.number}:${PhoneNumber('+' + v.number).getNumber('international')}\nEMAIL;type=Email:daffiawal07@gmail. com\nURL;type=Website:https://vipjasapanel.site\nADR;type=Location:;;🇯🇵 Jepang;;\nOther:${v.about}\nEND:VCARD`
      }))
      return diablo.sendMessage(jid, {
         contacts: {
